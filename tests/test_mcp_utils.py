...
from rlm_code.mcp.utils import format_mcp_error_message, is_closed_connection_error
...
class TestIsClosedConnectionError:
    ...
    @given(
        indicator=st.sampled_from(
            ["closed", "closedresourceerror", "Closed", "ClosedResourceError"]
        ),
...
class TestFormatMcpErrorMessage:
    ...
    def test_basic_formatting(self):
        ...
        message = format_mcp_error_message(
            "connect", "test-server", error, include_troubleshooting=False
        )
