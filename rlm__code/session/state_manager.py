"""
Session state management for RLM Code.
...
class SessionStateManager:
    ...
    def __init__(self, config_manager=None):
        ...
        primary_session_dir = Path.cwd() / ".rlm__code" / "sessions"
        legacy_session_dirs = [
            Path.cwd() / ".rlm_code" / "sessions",
            Path.cwd() / ".dspy_code" / "sessions",
        ]
        self.session_dir = primary_session_dir
        if not primary_session_dir.exists():
            for candidate in legacy_session_dirs:
                if candidate.exists():
