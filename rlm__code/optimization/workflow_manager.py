"""
Optimization workflow manager for RLM Code.
...
class OptimizationWorkflowManager:
    ...
    def __init__(self, code_generator=None, config_manager=None):
        ...
        primary_workflow_dir = Path.cwd() / ".rlm__code" / "optimization"
        legacy_workflow_dirs = [
            Path.cwd() / ".rlm_code" / "optimization",
            Path.cwd() / ".dspy_code" / "optimization",
        ]
        self.workflow_dir = primary_workflow_dir
        if not primary_workflow_dir.exists():
            for candidate in legacy_workflow_dirs:
                if candidate.exists():
                    self.workflow_dir = candidate
                    break
