"""
Main entry point for RLM Code.

This module launches the Textual TUI by default for creating, managing,
and optimizing agent workflows through slash commands and natural language.
"""
...
def check_safe_working_directory() -> None:
    """Check if current working directory is safe for rlm-code operations.
    ...
    if cwd == home:
        ...
        console.print(
            Panel.fit(
                ...
                "[yellow]You are running rlm-code from your home directory![/yellow]\n\n"
                ...
                "3. Install rlm-code: [cyan]pip install rlm-code[/cyan]\n"
                "4. Then run: [cyan]rlm-code[/cyan]\n\n"
            ...
            if parts and parts[0].lower() in dangerous_dirs:
                ...
                console.print(
                    Panel.fit(
                        ...
                        f"You are running rlm-code from: [cyan]{cwd}[/cyan]\n\n"
                        "This directory may contain personal files. For safety,\n"
                        "rlm-code will not scan files here.\n\n"
    ...
    # Also allow if we're in the repository root (has rlm__code package)
    is_repo_root = (cwd / "rlm__code").exists() and (cwd / "rlm__code" / "__init__.py").exists()
            ...
            if cwd == sys_dir or (cwd.is_relative_to(sys_dir) and not is_in_user_dir):
                ...
                console.print(
                    Panel.fit(
                        ...
                        f"You cannot run rlm-code from system directory: [cyan]{cwd}[/cyan]\n\n"
    ...
    if cwd == var_path or cwd == private_path:
        ...
        console.print(
            Panel.fit(
                "[bold red]🚨 CRITICAL ERROR[/bold red]\n\n"
                f"You cannot run rlm-code from: [cyan]{cwd}[/cyan]\n\n"
...
def get_version():
    """Get the installed version of rlm-code."""
    try:
...
def cli(verbose: bool, debug: bool, version: bool, skip_safety_check: bool):
    ...
    🚀 Welcome to RLM Code! This is a living playbook for agent systems.

    The CLI launches Textual TUI mode by default.
    ...
    RLM Code adapts to your stack and gives you framework-aware guidance
    ...
    if version:
        ...
        console.print(
            f"[bold cyan]RLM Code[/bold cyan] version: [green]{dspy_code_version}[/green]"
        )
        if dspy_version:
    ...
    # Launch TUI mode (default and only runtime path)
    try:
