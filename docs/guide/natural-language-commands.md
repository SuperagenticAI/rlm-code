...
RLM Code supports **natural language for all commands**! You don't need to remember slash commands - just describe what you want to do.
...
RLM Code automatically understands your intent and routes it to the appropriate command (for example, `/model` or `/connect`).
...
RLM Code: ✓ Project initialized

You: connect to ollama llama3.1:8b
RLM Code: ✓ Connected to ollama llama3.1:8b

You: create a sentiment analyzer
RLM Code: [Generates code...]

You: save the code as sentiment.py
RLM Code: ✓ Saved to sentiment.py

You: validate the code
RLM Code: ✓ Code is valid

You: run the code
RLM Code: [Executes code...]

You: generate 20 examples for sentiment analysis
RLM Code: [Generates training data...]

You: optimize the code
RLM Code: [Runs GEPA optimization...]

You: evaluate my program with accuracy
RLM Code: [Runs evaluation with accuracy metric...]

You: test performance of sentiment.py with test_data.jsonl
RLM Code: [Evaluates program with test data...]
```
...
RLM Code uses a **hybrid intelligent routing system**:
