...
Complete guide to managing DSPy projects with RLM Code, from initialization to deployment.

## Starting a New Project vs Existing Project

RLM Code supports two workflows:

1. **Blank Project** - Start from scratch
2. **Existing Project** - Add RLM Code to existing codebase
...
├── .dspy_code/               # rlm-code internal data
...
Next Steps:
1. Start interactive mode: rlm-code
...
Add RLM Code to an existing project:
...
rlm-code /init --name "my-project"

# Specify directory
rlm-code /init --path /path/to/project

# Configure model during init
rlm-code /init --provider ollama --model llama3.1:8b

# Verbose output
rlm-code /init --verbose
```
...
RLM Code automatically scans your project during `/init`:
...
You can still use RLM Code, but Q&A about code will be limited
```
...
git commit -m "Add RLM Code configuration"
```
...
Project management in RLM Code:
