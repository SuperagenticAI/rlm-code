...
Integrate RLM Code into your CI/CD pipeline for automated testing, validation, and deployment.

---

## Overview

This guide shows how to integrate RLM Code into continuous integration and continuous deployment pipelines.
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Validate Code
        run: |
          rlm-code /validate my_program.py
...
jobs:
  ...
  optimize:
    ...
    steps:
      ...
      - name: Install dependencies
        run: |
          pip install rlm-code dspy gepa
      - name: Optimize programs
        env:
          OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}
        run: |
          rlm-code /optimize my_program.py training_data.jsonl
...
```yaml
- name: Validate
  run: rlm-code /validate my_program.py
```
