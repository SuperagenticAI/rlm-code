...
Common issues and solutions for RLM Code.
...
```bash
rlm-code: command not found
```

**Solution:**
```bash
pip install --user rlm-code
...
2. **GitHub Issues**: [Report Issue](https://github.com/superagentic-ai/rlm-code/issues)
3. **Check Status**: `/status` to see current state
4. **Debug Mode**: Start with `rlm-code --debug`
