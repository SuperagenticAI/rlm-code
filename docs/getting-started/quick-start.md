...
Get started with RLM Code in 5 minutes! This guide will walk you through creating your first DSPy program.
...
# 4. Install rlm-code (always upgrade for latest features)
# Recommended: Use uv
...
# 5. (Optional but recommended) Install provider SDKs via rlm-code extras

# If you ONLY want local models (Ollama), you can skip this step.

# If using uv (recommended):
uv pip install "rlm-code[openai]"      # OpenAI support
uv pip install "rlm-code[gemini]"     # Google Gemini support
uv pip install "rlm-code[anthropic]"  # Anthropic (paid key required)
uv pip install "rlm-code[llm-all]"   # Or install all cloud providers at once
...
!!! tip "Always Activate Your Venv"
    Make sure your virtual environment is activated before running rlm-code. You should see `(.venv)` in your terminal prompt.
...
Before you do anything else in the CLI, you **must connect to a model**. RLM Code relies on an LLM for code generation and understanding.
...
!!! tip "What's Happening?"
    The `/init` command scans your installed DSPy version and your project code. This lets RLM Code answer questions about DSPy and understand your project!
...
RLM Code will execute your code in a safe sandbox and show you the results.
...
!!! info "Why Model Connection is Required"
    RLM Code needs an LLM to understand your requests and generate DSPy code. Without a connected model, most interactive features will not work.
...
RLM Code can answer questions about DSPy:
...
-   **📖 Understanding RLM Code**

    Learn how RLM Code works under the hood
