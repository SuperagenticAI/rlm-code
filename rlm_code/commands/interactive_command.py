"""
Interactive REPL mode for RLM Code.

This module provides a conversational interface where users can interact
with the CLI using natural language to generate code components.
"""
...
class InteractiveSession:
    """Manages an interactive REPL session for RLM Code."""
    ...
    def start(self):
        ...
        if not self._ensure_model_connected():
            console.print()
            console.print("[yellow]⚠️  Model connection is required to use RLM Code.[/yellow]")
            console.print()
            ...
            while True:
                try:
                    ...
                    user_input = get_user_input(
                        ...
                        slash_commands=sorted(self.slash_handler.commands.keys()),
                        session_status=self._build_prompt_status(),
                    )
    ...
    def _auto_connect_default_model(self):
        ...
        try:
            ...
            # Determine model type from explicit provider prefix or config fields.
            model_type = None
            api_key = None

            inferred = self.llm_connector.provider_registry.infer_provider_from_model(default_model)
            if inferred:
                model_type = inferred.provider_id
            elif default_model in self.config_manager.config.models.ollama_models:
                ...
                # Fallback to Ollama for backward compatibility with plain local model names.
    ...
    def _build_prompt_status(self) -> dict[str, str]:
        """Build prompt status metadata for the compact TUI bar."""
        model = self.llm_connector.current_model or "disconnected"
        provider = self.llm_connector.model_type or "-"

        return {
            "cwd": str(Path.cwd()),
            "model": model,
            "provider": provider,
            "messages": str(len(self.conversation_history)),
        }
...
"""
        ...
        console.print(Panel(Markdown(welcome_content), border_style="cyan", title="RLM Code"))
...
"""
    ...
    def _analyze_intent(self, user_input: str) -> str:
        ...
        # PRIORITY 7: Check for retrieval-specific requests
        ...
        if any(keyword in user_input_lower for keyword in rag_keywords) and not is_question:
            logger.debug("Intent: create_module (matched retrieval keywords)")
            return "create_module"
    ...
    def _handle_create_signature(self, user_input: str):
        ...
        console.print("I'll help you create a signature/interface. Let me extract the details...\n")
    ...
    def _handle_create_module(self, user_input: str):
        ...
        console.print("I'll create a module for you!\n")
        ...
        if not module_code or not module_code.strip():
            ...
            if not self.llm_connector.current_model:
                ...
                console.print('  • "Create a retrieval system with document search"')
    ...
    def _handle_create_program(self, user_input: str):
        ...
        console.print("I'll create a complete program for you!\n")
        ...
        if not complete_code or not complete_code.strip():
            ...
            if not self.llm_connector.current_model:
                ...
                console.print('  • "Build a question answering system with retrieval"')
        ...
        # Display the code beautifully
        show_code_panel(complete_code, "Generated Complete Program", "python")
    ...
    def _show_lm_configuration_hint(self) -> None:
        ...
        so they can easily align DSPy LM configuration with the model connected in RLM Code.
        """
    ...
    def _handle_explain(self, user_input: str):
        ...
        if matched_topic:
            ...
            console.print('  • "Explain retrieval workflows"')
...
Framework Reference (DSPy included):
...
- If they want code: Generate complete, executable code in the framework they requested
- Do NOT default to DSPy unless explicitly requested
- If no framework is specified, prefer framework-neutral Python
...
Be conversational, helpful, and use the context to provide accurate information."""
        ...
        if self._looks_like_code_response(response):
            # Extract and display code
            code = self._extract_code_from_response(response)
            if code and code.strip():
                self.current_context["last_generated"] = code
                self.current_context["type"] = "module"
                show_code_panel(code, "Generated Code", "python")
    ...
    def _build_context(self, user_input: str, reference: str) -> dict[str, Any]:
        """Build comprehensive context dictionary for model responses.
...
You have access to:
- Real framework source code examples from the user's installed version
- Framework reference documentation (DSPy included)
- Conversation history
...
3. Use the syntax/patterns for the framework explicitly requested by the user
...
7. Do NOT default to DSPy unless the user explicitly asks for DSPy
...
SIGNATURE/INTERFACE PATTERNS (use codebase examples as reference):
...
- The codebase examples below are from actual source code - follow their style when relevant

=== REAL FRAMEWORK SOURCE CODE EXAMPLES (Primary Reference) ===
{context.get("codebase_examples", "No codebase examples available - use standard Python patterns")}
...
9. Use DSPy only when the user explicitly requests DSPy behavior
...
Generate code that ACTUALLY solves the user's specific problem using the requested framework!"""
    ...
    def _generate_module_with_llm(self, user_input: str) -> str:
        """Generate module code using connected LLM with framework context.
        ...
        # ALWAYS use LLM with framework context
        if self.llm_connector.current_model:
            try:
...
You have access to:
- Real framework source code from the user's installed version (via codebase examples)
- Framework reference documentation (DSPy included)
- Conversation history
- Best practices from the codebases

CRITICAL RULES - YOU MUST FOLLOW THESE EXACTLY:
1. Output ONLY valid, executable Python code - NO markdown, NO explanations, NO pseudo code
2. Generate COMPLETE, RUNNABLE code that can be executed immediately
3. Follow the framework requested by the user; do NOT default to DSPy unless explicitly requested
4. Understand the user's ACTUAL intent - analyze their request carefully
5. Include proper imports and complete, production-ready implementation
6. Follow patterns from the provided codebase examples
...
9. Generate REAL, WORKING code that can be copied and run immediately"""
...
- Template examples are provided as REFERENCE/GUIDANCE only - adapt them, don't copy them
- Generate REAL, WORKING code tailored to the user's specific request - NOT generic templates
- The codebase examples below are from actual framework/GEPA/MCP source code - follow their style exactly
...
9. Use DSPy only if the user explicitly asks for DSPy
...
Generate code that ACTUALLY solves the user's specific problem in the requested framework!"""
                ...
                if code and code.strip():
                    ...
                    if has_incomplete_forward or has_not_implemented or has_placeholder:
                        ...
                        retry_prompt = f"""The previous attempt had incomplete code.
Regenerate FULL, WORKING, EXECUTABLE Python code for this exact request:

{user_input}

Requirements:
1. Fully implemented methods (NO 'pass', NO placeholders)
2. Include required imports and runnable structure
3. Follow the framework requested by the user
4. Do NOT default to DSPy unless explicitly requested

Relevant codebase examples:
{context.get("codebase_examples", "")}

Generate ONLY complete Python code - no explanations, no markdown."""
                        ...
                        try:
                            ...
                            if code and code.strip():
                                ...
                                has_class_or_def = "class " in code or "def " in code
                                has_forward = "def forward" in code

                                if has_class_or_def and (has_import or has_forward):
                                    logger.debug(
                    ...
                    if self._looks_like_code_response(code):
                        # Basic validation: has structure
                        has_import = bool(re.search(r"^\s*(import|from)\s+\w+", code, re.MULTILINE))
                        has_class_or_def = "class " in code or "def " in code

                        if has_class_or_def or has_import:
                            logger.debug(f"Successfully generated {len(code)} characters of code")
                            return code
                        else:
                            logger.warning(
                                "Code missing basic structure: "
                                f"import={has_import}, class_or_def={has_class_or_def}"
                            )
                    else:
                        ...
                        if code_blocks:
                            ...
                            # Basic validation
                            if self._looks_like_code_response(extracted):
                                return extracted
...
'''
    ...
    def _extract_code_from_response(self, response: str) -> str:
        ...
        # Look for fenced code blocks first (with or without explicit python language tag)
        ...
        # Look for likely Python code without fences
        lines = response.split("\n")
        start_idx = None
        for idx, line in enumerate(lines):
            if re.match(r"^\s*(import|from|class|def|async def)\b", line):
                start_idx = idx
                break

        if start_idx is not None:
            candidate = "\n".join(lines[start_idx:]).strip()
            if self._looks_like_code_response(candidate):
                return candidate

        return response.strip()

    def _looks_like_code_response(self, text: str) -> bool:
        """Heuristic check for code-like LLM responses."""
        import re
...
def _show_welcome_screen(console, context, config_manager):
    """Show startup dashboard for interactive mode."""
    ...
    from ..ui.welcome import RLM_ASCII_ART
    ...
    ascii_panel = Panel(
        Align.center(Text(RLM_ASCII_ART, style="bold cyan")),
        border_style="cyan",
        box=ROUNDED,
        padding=(0, 2),
    )
    ...
    title = Text()
    title.append("RLM Code ", style="bold cyan")
    title.append("Interactive Harness", style="bold white")
    console.print(Align.center(title))
    console.print()

    model_name = config_manager.config.default_model if config_manager else None
    ...
    quick = Table(show_header=False, box=None, pad_edge=False)
    quick.add_column(style="cyan", width=22)
    quick.add_column(style="dim")
    quick.add_row("/model", "Interactive model picker")
    quick.add_row("/connect <provider> <model>", "Connect directly")
    quick.add_row("/init", "Scan/init project context")
    quick.add_row("/status", "Show session context and outputs")
    quick.add_row("/help", "All commands")

    workflows = Table(show_header=False, box=None, pad_edge=False)
    workflows.add_column(style="green", width=12)
    workflows.add_column(style="dim")
    workflows.add_row("Build", '"Create a classifier for support tickets"')
    workflows.add_row("Data", '"/data support-triage 30"')
    workflows.add_row("Optimize", '"/optimize my_program.py"')
    workflows.add_row("Validate", '"/validate" then "/run"')

    panels = [
        Panel(status_table, title="Session", border_style="cyan", box=ROUNDED),
        Panel(quick, title="Commands", border_style="yellow", box=ROUNDED),
        Panel(workflows, title="Workflows", border_style="green", box=ROUNDED),
    ]
    console.print(Columns(panels, equal=True, expand=True))
    console.print()

    model_info = Text()
    model_info.append("Tip: ", style="bold yellow")
    model_info.append("use Tab to complete slash commands", style="dim")
    model_info.append(" | ")
    model_info.append("use /models cloud for provider matrix", style="dim")
    console.print(model_info)
    console.print("[dim]Start typing your request or run /help[/dim]")
    console.print()
