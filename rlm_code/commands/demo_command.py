...
def execute_demo(config_manager=None, demo_type="basic"):
    """
    Run a complete working demo of RLM Code.
    ...
    console.print("\n[bold cyan]🎬 RLM Code Demo - Email Classification[/bold cyan]\n")
...
def execute_mcp_demo(config_manager=None):
    ...
    console.print(
        "\n[bold cyan]🎬 RLM Code MCP Demo - Document Analyzer with Filesystem[/bold cyan]\n"
    )
...
1. [yellow]Configure filesystem MCP:[/yellow]
   Edit rlm_config.yaml (legacy: dspy_config.yaml) and add the filesystem server
...
[dim]💡 Tip: MCP servers are configured in rlm_config.yaml (legacy: dspy_config.yaml)
