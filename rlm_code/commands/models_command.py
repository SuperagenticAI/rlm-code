...
def list_models(verbose: bool = False) -> None:
    ...
    if not config_manager.is_project_initialized():
        console.print("[red]Error:[/red] Not in a DSPy project directory.")
        console.print("Run 'rlm-code init' (or 'rlm-code init') to initialize a new project.")
        return
    ...
    try:
        ...
        if not any(
            ...
            console.print(
                "Add models by editing rlm_config.yaml (legacy: dspy_config.yaml) or using /connect in interactive mode."
            )
...
def set_default_model(model_name: str, verbose: bool = False) -> None:
    ...
    try:
        ...
        if not model_manager.is_model_configured(model_name):
            ...
            console.print(
                "Define it in rlm_config.yaml (legacy: dspy_config.yaml) or connect via /connect in interactive mode first."
            )
            return
